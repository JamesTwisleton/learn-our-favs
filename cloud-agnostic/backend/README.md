...
- The docker-compose stack from `../docker-compose.yml` running (Postgres,
...
| `catalogue.matching` | `TitleNormalizer` + `TrigramSimilarity` + `SongMatcher` (ADR 0013). Mirrors `lean/src/lib/matching.ts`. | done + tested |
...
live in `../workers`), the AWS/GCP `platform.*` implementations. Order of work:
`../docs/READ-IN-THIS-ORDER.md`.
